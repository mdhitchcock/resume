...
* Asset bundler and minimizer (Javascript and CSS)
...
* Homepage redesign (2008)
  ...
  * also was the team lead on ensuring the Visual Sciences/Omniture metrics were all coded properly and delivering expected data to the metrics team
* Component Library
  ...
  * wrote framework code to dynamically load component Javascript and CSS based on page markup
...
* Single Point of Contact (SPoC) for Product Category Redesign project
  ...
  * to maintain clarity, I wrote overview docs, many UE specs and "speclets", and got signoff from the stakeholders and met with team members to review them
* Homepage redesign (2010)
  * Used Basecamp to work with the third party design vendor (37signals)
  * worked with the ID team to upgrade the UI and layout
  * scripted a process to package the current revision of the homepage project into a "DevKit" ("homepage in a box") to pass between our team and the design vendor
* Workspace/Portlets
  ...
  * I worked on a UE redesign to improve the layout of the portlets, offer switching between several views, and allow drag-and-drop
...
* Libra to Masterbrand migration
  ...
  * so to change the UE design simultaneously across the production site, the entire team had to do meticulous development and testing
...
* Masterbrand 2.0
  * this was a UE design upgrade, as well as bugfixes from the initial migration -- mostly Javascript and CSS work
* Bluenote
  * SHP (Support homepage) and Support MegaMenu optimization and UE upgrade
...
* Click to Chat
  ...
  * the code had to scan the page for chat button markup; if that was found, then determine the correct locale, pull in a JSON config file for button parameters based on the locale,  pull in Salesforce code from their site, and finally wire up the button to poll the Salesforce API to determine if agents were online
  * almost all of the steps were asynchronous, so I got up to speed on Promises and fell in love with them
  * when the cutover from Liveperson to Liveagent occurred, the new code was ready and worked largely as expected
...
* On a daily basis I worked closely with Web Development to help them with ever-growing customer requirements by designing and coding new features or enhancements to adapt the Forms Engine or the forms markup language as needed.  I released new code to production almost daily and wrote tools that are still in use to streamline a highly intensive work flow.  It was very satisfying to be able to support my team in this way, especially when they were part of the process.
...
* Introduced bug tracking company wide -- installed and configured Bugzilla, advocated and educated others on it; required all requests for IT and Forms Engine be made via Bugzilla; it became indispensable to the development process
